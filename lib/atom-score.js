...
import { init, actions, select, reducer } from './state/index';
import reducer2, { update, INIT } from './state.js';
...
export default {
  ...
  activate (state) {
    ...
    // Initialize redux
    this.revert(state);
    this.newStore = createStore(reducer2, INIT);
  ...
  dispatch (stim) {
    ...
    this.updateView();
    // TODO: remove the old store after it's all replaced
    this.newStore.dispatch(update(stim));
    const st = this.newStore.getState();
    console.log('st', st);
  },
