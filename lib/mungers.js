...
export const lastTwoCommands = lastNCommands(2)
export const predict         = (state) => {
  if (!state.recent || state.recent.size < 3) { return }
  const lastStrokes  = lastTwoCommands(state).map(c => c.id)
  if (lastStrokes.size < 2) { return }
