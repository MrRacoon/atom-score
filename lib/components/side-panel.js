...
class SidePanel extends HTMLElement {
  ...
  setState(state) {
    const s   = selector.score(state);
    const sh  = selector.high(state);

    const st  = selector.streak(state);
    const sth = selector.streakH(state);
    const stl = selector.streakL(state);

    const pts = selector.lastPoints(state);
    const avg = selector.avgScore(state);
    const c   = (pts > 0) ? 'green' : (pts < 0) ? 'red' : '';

    const pp = selector.predictedStim(state) || {};

    this.score.setValue(`${s} ${pts >= 0 ? '+'+pts : pts} (${sh})`);
    this.streak.setValue(`${st} (${stl}/${sth})`);
    this.avg.setValue(avg);
    this.counter.setValue(selector.count(state));
    this.recent.setValue(selector.comboString(state));
    this.last.setValue(selector.lastCommand(state));
